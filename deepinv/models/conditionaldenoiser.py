...
class ConditionalDenoiser(nn.Module):
    r"""
    Conditial denoiser architecture
    ...
    def forward(self, x, sigma, c, **kwargs):
        r"""
        Reconstructs a signal estimate from a noisy input x and the conditional information c

        :param torch.tensor x: noisy input
        :param torch.tensor c: conditional information

        """
        # TODO : make UNetRes compatible with conditional denoising
        if type(self.backbone_net).__name__ == "UNetRes":
            noise_level_map = (
